...
fn init_tracing(log_level: String, name: String) -> tracing_appender::non_blocking::WorkerGuard {
    ...
    println!("📝 Logging to: {}", std::fs::canonicalize(&log_file).unwrap_or_else(|_| log_file.clone().into()).display());
...
async fn main() -> anyhow::Result<()> {
    ...
    let (rpc_client, rpc_sender_task, rpc_listener_task) = RpcClient::connect( L2_ID, BITVMX_ID, config.bitvmx.broker_port, None,Some(&shutdown_tx));
